...
- **Wishlist UX**: Heart taps must work without login by saving to `localStorage` first, then best-effort syncing to `/api/wishlist`; never store or POST base64 images for wishlist items, use lightweight image URLs.
