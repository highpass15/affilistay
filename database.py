...
+        ''')
+    else:
+        # SQLite (스키마는 동일하므로 생략하거나 최소한으로 유지)
...
+        if DATABASE_URL:
+            cursor.execute('''
+                SELECT o.id, o.total_amount, o.settlement_status, p.owner_id, h.name as host_name
+                FROM orders o JOIN products p ON o.product_id = p.id
+                JOIN hosts h ON p.owner_id = h.id WHERE o.id = %s
+            ''', (order_id,))
+        else:
