...
# --- 영구 주소 설정 ---
CHECKOUT_URL = "https://affilistay-showroom.com"
...
def check_auth():
    ...
    with col_m:
        ...
        if st.session_state['auth_mode'] == 'login':
            ...
            phone = st.text_input("핸드폰 연락처(통신사 인증)")
            email = st.text_input("이메일 주소")
            
            signup_path_options = ["네이버광고", "지인소개", "인스타", "메일광고", "기타"]
            signup_path = st.selectbox("가입경로", signup_path_options)
                ...
                try:
                    if database.DATABASE_URL:
                        cursor.execute(
                            ...
                            (new_username, new_password, full_name, phone, email, signup_path, role_choice, desired_prod, False)
                        )
                    else:
                        cursor.execute(
                    ...
                    conn.commit()
                    st.success("가입 신청이 완료되었습니다. 로그인 해주세요.")
                    st.session_state['auth_mode'] = 'login'
                    st.rerun()
...
if st.session_state['role'] == 'GUEST':
    st.title(f"Welcome, {st.session_state['name']}님!")
    st.markdown("---")
...
with tab_list[0]:
    ...
    if st.session_state['is_master']:
        ...
        params = (st.session_state['host_id'],)
...
if st.session_state['is_master']:
    with tab_list[3]:
        st.title("💰 정산 관리 시스템")
        st.markdown("<p style='color: #888;'>판매가 기준 <b>플랫폼 20%(카드수수료 포함) / 호스트 수익 10%</b> 자동 계산 시스템입니다.</p>", unsafe_allow_html=True)
