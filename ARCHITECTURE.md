...
## 1. 프로젝트 개요
AFFILISTAY는 숙소 호스트(Host)가 협찬받은 프리미엄 제품을 객실(거실, 침실, 주방, 화장실 등)에 비치하고, 게스트(Guest)가 QR 코드를 스캔하여 제품을 체험하고 현장에서 즉시 구매할 수 있도록 돕는 **O2O(Online to Offline) 쇼룸 커머스 플랫폼**입니다.
...
## 5. 핵심 사용자 플로우
1.  **호스트 세팅 (Streamlit)**: 
    *   어드민 대시보드에 로그인하여 협찬받은 상품을 등록합니다. (제품명, 가격, 사진, 배치 위치 등)
